				...
				unsigned long event, void *dummy)
{
	...
	case PM_SUSPEND_PREPARE:
		if (!pm_runtime_status_suspended(&srp.pdev->dev)) {
			srp.pm_noti_suspended = PM_STAT_RUNTIME;
			pm_runtime_suspend(&srp.pdev->dev);
		} else if (!srp.pm_suspended) {
			srp.pm_noti_suspended = PM_STAT_SLEEP;
			pm_runtime_get_sync(&srp.pdev->dev);
			srp_core_suspend(RUNTIME);
		}
		srp_info("%s : to %d\n", __func__, srp.pm_noti_suspended);
		return NOTIFY_OK;
	case PM_POST_HIBERNATION:
	case PM_POST_SUSPEND:
		srp_info("%s : from %d\n", __func__, srp.pm_noti_suspended);
		if (srp.pm_noti_suspended == PM_STAT_RUNTIME) {
			pm_runtime_resume(&srp.pdev->dev);
		} else if (srp.pm_noti_suspended == PM_STAT_SLEEP) {
			srp_core_reset();
			pm_runtime_put_sync(&srp.pdev->dev);
		}
		srp.pm_noti_suspended = PM_STAT_NONE;
		return NOTIFY_OK;
	default:
		return NOTIFY_DONE;
	}
...
static int srp_runtime_resume(struct device *dev)
{
	...
	if (srp.pm_noti_suspended == PM_STAT_NONE)
		srp_core_resume();
